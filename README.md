...
3) Запустить контейнер.
   ...
   docker-compose up
   ```
4) Дождаться полной инициализации всех сервисов
   
   Celery запускается достаточно долго из-за инициализации CTCPipeline, который отвечает за преобразование аудио в текст, но т.к. вебсервер запускается одновременно с celery может возникнуть проблема, что задача уже отправилась на сервис, а сам сервис еще полноценно не включен.
...
3) 500 код ошибки при отправке любых данных на endpoint.
   ...
   Может быть вызвано тем, что celery не до конца запустился, данные будут обработаны сразу после запуска, т.к. сначала они записываются в БД.
   
   В скором времени планируется изменения, для запуска веб-приложения только после полной инициализации celery, во избежания этой проблемы в будущем.
