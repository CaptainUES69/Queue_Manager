    ...
    model_config = {
        "json_schema_extra": {
            "examples": [
                ...
                {"status": StatesAPI.success.value, "data": [["text"], ["from file"]]},
            ]
        }
    }
...
class ResponseError(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {
            "examples": [
                {"status": StatesAPI.error.value, "message": "Something about trouble"}
            ]
        }
    }
...
def audio_file_input(file: UploadFile, callback_url: str = None) -> JSONResponse:
    ...
    _id = TableManager.create_task_backup(
        custom_id, file_path=file_path, callback_url=callback_url
    )
    if _id:
        return JSONResponse(
            content={
        ...
        if not isfile(file_path):
            logger.critical(
                f"Unable to save received file: {file_path} with task id: {custom_id}"
            )
            return JSONResponse(
                content={
...
def audio_url_input(file_url: str, callback_url: str = None) -> JSONResponse:
    ...
    _id = TableManager.create_task_backup(
        custom_id, file_url, callback_url=callback_url
    )
    if _id:
        return JSONResponse(
            content={
...
@app.post(
    ...
    responses={
        ...
        status.HTTP_400_BAD_REQUEST: {
            "model": ResponseError,
            "description": "Something about trouble",
        },
    },
...
async def trancribe_audio(
    ...
    callback_url: Optional[str] = Form(
        None, examples=["https://example.com/webhook"], description="Return data URL"
    ),
    ...
    if callback_url:
        validated = AnyHttpUrl(
            callback_url
        )  # Если не провалидирует то вернет ошибку 422
...
@app.get(
    path="/transcribe_audio/get_data",
    tags=["Get data from task"],
    responses={
        status.HTTP_200_OK: {
            "model": ResponseData,
            "description": "Return data from task",
        },
        status.HTTP_404_NOT_FOUND: {
            "model": ResponseError,
            "description": "Task doesn`t exists",
        },
    },
)
async def get_data_from_task(
    task_id: str, bg: BackgroundTasks
) -> ResponseData | ResponseError:
    ...
    if task.status == StatesTasks.success.value:
        ...
        return ResponseData(
            status=StatesAPI.success.value, data=pickle.loads(task.result)
        )

    logger.info(f"Return status of task with id: {task_id}")
    return ResponseData(status=StatesAPI.success.value, data=task.status)
