...
def load_backups() -> None:
    ...
    for backup in backups:
        ...
        transcribation_task.apply_async(args = [filepath, backup.callback_url, backup.task_id], task_id = backup.task_id)
...
def transcribe_file(self: Task, audiopath: str, log_id: str = None) -> list[str]:
    try:
        ...
        if exists(audiopath) and isfile(audiopath):
            remove(audiopath)
...
def callback_result_to_url(self: Task, callback_url: str, result: list[str], log_id: str = None) -> None:
    if callback_url:
...
def transcribation_task(self: Task, audio_location: str, callback_url: str, log_id: str = None) -> list[str]:
    ...
    if exists(audio_location) and isfile(audio_location):
