...
class TestTasksFunctions:
    ...
    def test_create_pipeline_local_model(self):
    ...
    def test_create_pipeline_huggingface_model(
        self, mock_tasks_os_path: Generator[MagicMock | AsyncMock, Any, None]
    ):
        mock_tasks_os_path.return_value = False
        result = create_pipeline()
    ...
    def test_download_file_success(
        ...
        result = download_file(
            mock_self_task, "http://example.com/audio.mp3", "test-log-id"
        )
    ...
    def test_callback_success(
        ...
        # Act
        callback_result_to_url(
            mock_self_task, "http://callback.example.com", result_data, "test-log-id"
        )

        # Assert
        mock_tasks_requests.post.assert_called_once_with(
            "http://callback.example.com", json=result_data
        )
        mock_self_task.update_state.assert_called_once_with(
            state=StatesTasks.callback.value
        )
    ...
    def test_load_backups_success(
        ...
        backup1 = backup_task_factory(
            task_id="backup-1", file_path="/path/to/file1.mp3"
        )
        ...
        celery_task_success = celery_task_factory(
            task_id="celery-success", status=StatesTasks.success.value
        )

        celery_task_failed = celery_task_factory(
            task_id="celery-failed", status=StatesTasks.decode_exc.value
        )
    ...
    def test_load_backups_already_executed(
        ...
        # Arrange
        backup = backup_task_factory(task_id="backup-1", file_path="/path/to/file.mp3")

        celery_task = celery_task_factory(
            task_id="backup-1", status=StatesTasks.success.value
        )
