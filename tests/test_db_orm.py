...
class TestDataBaseORM:
    ...
    def test_get_task(self, _id: str, conf_TB, table: Type[T]) -> T | None:
    ...
    def test_get_all_task(self, conf_TB, table: Type[T]) -> list[T] | None:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (str(uuid.uuid4()), 'http://free_flac.com', 'tests/files/testfile.flac', None),
    ...
    def test_create_task_backup(
        ...
        task = session_maker().query(BackupTasks).filter(BackupTasks.task_id == _id).first()
    ...
    def test_delete_task_backup(self, session_maker: sessionmaker[Session], _id: str) -> str | None:
